...
- (instancetype)initWithGraph:(NSArray*)point
{
	return [self initWithGraph:point andColor:[NSColor whiteColor]];
}

- (instancetype)initWithGraph:(NSArray*)point andColor:(NSColor*)color
{
	self = [super init];
	if (!self)
	{
...
- (void)drawSubAtPoint:(NSPoint)p
{
	...
	for (NSInteger i = 2; i < [_graph count]; i+=2)
	{
