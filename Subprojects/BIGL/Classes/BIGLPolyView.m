...
- (instancetype)initWithPolygon:(NSArray*)p
{
	return [self initWithPolygon:p andColor:[NSColor whiteColor]];
}

- (instancetype)initWithPolygon:(NSArray*)p andColor:(NSColor*)c
{
	self = [super init];
	if (!self)
	{
...
- (void)drawSubAtPoint:(NSPoint)p
{
	...
	for(NSInteger i = 0; i < [_polygon count]; i+=2)
	{
...
- (void)drawCocoaSubAtPoint:(NSPoint)p
{
	...
	for(NSInteger i = 2; i < [_polygon count]; i+=2)
	{
