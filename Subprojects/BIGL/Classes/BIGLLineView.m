...
{
	return [self initWithLines:lines andColor:[NSColor whiteColor]];
}

- (instancetype)initWithLines:(NSArray*)lines andColor:(NSColor*)color
{
	self = [super init];
	if (!self) return nil;
...
- (void)drawSubAtPoint:(NSPoint)p
{
	...
	for (NSInteger i = 0; i < [_lines count]; i+=4)
	{
		glBegin(GL_LINES);
